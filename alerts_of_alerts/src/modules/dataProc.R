dataProcModule <- function(input, output, session, sideBarInput) {
  ns <- session$ns
  
  ccdd_list <- reactive({
    "https://essence.syndromicsurveillance.org/nssp_essence/servlet/SyndromeDefinitionsServlet_CCDD?action=getCCDDTerms" %>%
  ...
  fips_for_url <- reactive({
    county_sf %>%
      rename(state_fips = STATEFP) %>%
      left_join(state_helper, by = "state_fips") %>%
      if (sideBarInput()$state != "All") {
        {
          filter(state_name == sideBarInput()$state)
        } %>%
          pull(GEOID) %>%
          as.character() %>%
  ...
  category_for_url <- reactive({
    ccdd_list() %>%
      filter(category == sideBarInput()$ccdd) %>%
      pull(category_api)
  })

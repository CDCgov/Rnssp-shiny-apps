...
```{r plotlyout, echo=FALSE, message=FALSE, fig.height=4.05}

updated_plotly_object <- params$plotly_object %>%
  layout(
    width = params$plotly_width,
    height = params$plotly_height,
    margin = list(t = 0, b = 0, l = 0, r = 0)  # Top, bottom, left, right margins
  )

updated_plotly_object

# Add the caption manually as markdown text
p()
p(HTML("<strong>Alerts of alerts time series diagnostics</strong> (top panel) total statewide percent of ED visits, (middle panel) number of alerting counties/regions, (bottom panel) the number of counties/regions estimated to have increasing case counts. The top and middle panels use the <a href='https://github.com/CDCgov/Rnssp' target='_blank'>Rnssp Switch (Adaptive Regression/EWMA)</a> alert detection algorithm to test for Alerts. The bottom panel uses an estimated slope threshold to signal Alerts. The fixed, vertical dashed line (black) indicates the date for which data are plotted as a family of choropleth maps below and on which attendant spatial auto-correlation test are run."))
```
...
<div class="modified-text">
  <strong>a. </strong>Spatial distribution of p-values &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong>b. </strong>Spatial distribution of Alerts &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong>c. </strong>Spatial Distribution of Trends
</div>
...
*The template for this report was originally created by [Josh Kimrey](https://github.com/jkimrey13), consolidated, improved, and adapted to the Rnssp package by [Gbedegnon Roseric Azondekon](https://rosericazondekon.github.io). For questions, ideas for improvement/collaboration, or attribution, please submit an issue at <https://github.com/CDCgov/Rnssp-shiny-apps/issues>.*

...
<div style="text-align: center; font-size: 0.8em;">
  `r paste0("Report generated on ", format(Sys.Date(), "%A, %B %d, %Y")," at ", format(Sys.time(), "%X %Y %Z"),".")`
</div>
...
# Create a data frame
data <- data.frame(
  Variables = c("State", "CC & DD Category", "Start Date", "End Date", "Maps Date"),
...
#kable(data, col.names = NULL, escape = FALSE) %>%
#  kable_styling(bootstrap_options = c("condensed"), full_width = FALSE) %>%
#  add_header_above(c("Selections" = 2)) %>%
#  column_spec(1, italic = TRUE) #%>%
  #column_spec(2, border_left = TRUE, border_right = TRUE)

# Generate table without column names
#kable(data, col.names = NULL) %>%
#  kable_styling(full_width = FALSE) %>%
#  add_header_above(c(" " = 1, "User Selections" = 1)) %>%

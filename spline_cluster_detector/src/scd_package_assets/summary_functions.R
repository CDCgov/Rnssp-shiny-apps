...
#' @param plot_type string indicating either a "ggplot" or "plotly" result
#' @param locations string indicating location name (defaults to "All Locations")
#' @param ... passed onto plotly
#' @export
#' @returns a ggplot or plotly object
...
generate_time_series_plot <- function(
  ...
  if (plot_type == "ggplot") {
    p <- generate_ggplot_time_series(
      time_series_data[date <= end_date], locations=locations
    )
  }

  if (plot_type == "plotly") {
    p <- generate_plotly_time_series(
      time_series_data[date <= end_date], locations=locations, ...
    )
  }
  return(p)
...
generate_ggplot_time_series <- function(
    time_series_data, locations="All Locations") {
  date_count <- NULL
  time_series_plot <- ggplot2::ggplot(
    ...
    ggplot2::scale_x_date(date_labels = "%b-%d-%y", date_breaks = "2 weeks") +
    ggplot2::xlab("Date") +
    ggplot2::ggtitle(paste0("Daily Record Count for ", locations))
...
generate_plotly_time_series <- function(
  ...
  time_series_plot <- plotly::plot_ly(
    ...
    plotly::layout(
      title = paste0("Daily Record Count for ", locations),
